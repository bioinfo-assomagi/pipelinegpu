...
class CoveragePipe(ParallelPipe):
    ...
    def process(self, **kwargs):
        ...
        for sample in samples:
            ...
            if sospetto == 'CONNETTIVOPATIE':
                ...
                vertical, verticalX = cutCDS_jurgen.create_vertical(bed, bedx, sample_name, folder_pheno)
                print(vertical)
            ...
            if not vertical.empty:
                ...
                try: 
                    ...
                    print('Cannot read vertical macro') #verti
                    print(e)
                sample_name = count_unbalance(genome, vertical, verticalX, principal_directory, sample, phenotype, vertical_macro)
                ...
                folder_to_count_file = join(principal_directory,'temp/','to_count/',sample_name+'*_disease')
                glob_ = glob.glob(folder_to_count_file)
                glob_ = str(glob_)[1:-1].replace(',',' ')
                glob_ = str(glob_)[1:-1].replace('\'','')
                os.system(' '.join(['cat',glob_,'>',join(principal_directory,'temp/',sample_name+'_final_disease')]))
                name = join(principal_directory,'temp/',sample_name+'_final_disease')
                count = pd.read_csv(name,sep='\t',header=None,names=cols)
                count.fillna(0,inplace=True)

                folder_to_count_sex = join(principal_directory,'temp/','to_count/',sample_name+'*_SEX')
                glob_sex = glob.glob(folder_to_count_sex)
                glob_sex = str(glob_sex)[1:-1].replace(',',' ')
                glob_sex = str(glob_sex)[1:-1].replace('\'','')
                os.system(' '.join(['cat',glob_sex,'>',join(principal_directory,'temp/',sample_name+'_final_sex')]))
                ...
                filter_disease_files(BED,vertical,principal_directory,phenotype,count,sample_name,buchiartificiali)

                file_to_count = join(principal_directory,'temp/',sample_name+'_to_count')
                ...
                try: 
                    filter_macroarea_files(vertical_macro,principal_directory,sample_name,buchiartificiali)
                except: 
                    filter_macroarea_files(vertical,principal_directory,sample_name,buchiartificiali)
                print('outplot')
                print(join(principal_directory,'coverage/',sample_name,sample_name+'_final_sex'))
    ...
    def cutCDS(self, genelist, sample, folder_pheno, server_id):
        ...
        vertical, verticalX, BED = cutCDS_jurgen.cutCDS(genelist, sample_name, folder_pheno, server_id)
...
def count_unbalance(genome,vertical,verticalX,folder, sample, phenotype, vertical_macro):
 
    sample_name = str(sample['name'])
    bam = sample['bam']

    file_to_count = join(folder,'temp/',sample_name+'_to_count')
    file_vcf = join(folder,'temp/',sample_name+'_samt.vcf')

    # OPTIONAL to replace by parabricks
    os.system(' '.join(['samtools','mpileup',
    ...
    if not os.path.exists(folder_to_count):
        os.makedirs(folder_to_count, exist_ok=True) # TODO: remove the ifs, keep only: exist_ok=True
    folder_to_macroarea = join(folder,'temp/','to_macroarea/')
    if not os.path.exists(folder_to_macroarea):
        os.makedirs(folder_to_macroarea, exist_ok=True)
    ...
    for chunk in a:
        ...
        try: filter_for_macroarea(vertical_macro,folder,sample_name,i)
        except: filter_for_macroarea(verticalX,folder,sample_name,i)
